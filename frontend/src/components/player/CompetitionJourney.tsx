...
export default function CompetitionJourney({ achievements, historical }: Props) {
  ...
  const grouped = historical
    ? achievements.reduce<Record<string, PlayerCompetitionAchievement[]>>(
        (groups, achievement) => {
          ...
          <div className="competition-journey__group" key={season}>
            ...
            <div className="competition-journey__list">
              {items.map((achievement) => {
                ...
                const achievementSeason = competition === 'Mundial'
                  ? `Mundial ${achievement.season}`
                  : seasonLabel(achievement.season)
                ...
                return (
                  <article
                    ...
                    tabIndex={0}
                    aria-label={`${phase} de ${competition}. ${teamName}, ${achievementSeason}. ${bonus}.`}
                    onFocus={(event) => event.currentTarget.scrollIntoView({ block: 'nearest', inline: 'center' })}
                  >
                    <span className="competition-journey__summary" aria-hidden="true">
                    ...
                    <span className="competition-journey__detail" aria-hidden="true">
                      <small>{teamName}</small>
                      <span>{achievementSeason}</span>
                      <strong>{bonus}</strong>
